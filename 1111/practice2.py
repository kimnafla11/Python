        ...
        if GPIO.input(BUTTON):#버튼이 눌렸을때 실행
            ...
            if  now == "green" :  #now변수 값이 green이면 아래 문장 실행
                ...
                now = "yellow" # 지금 상태를 표시하는 변수에 yellow값 저장
                 ...
                 time.sleep(1) #1초동안 노란불 지속
                 if GPIO.input(BUTTON):#1초 내에 버튼을 눌렀을때
                    GPIO.output(LED_YELLOW,False)#LED_YELLOW 디지털 출력을 LOW로 설정- 노란불 끔
                    break #break문으로 while문 빠져나옴 -종료
                 GPIO.output(LED_YELLOW,False)#노란 불 끔
